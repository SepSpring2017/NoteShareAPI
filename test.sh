...
# Passing the ACCESS_TOKEN as an Authorization header and quering the list of all users
curl -v -H "Authorization: Bearer $ACCESS_TOKEN" http://localhost:5000/api/users

# Query the current user
curl -v -H "Authorization: Bearer $ACCESS_TOKEN" http://localhost:5000/api/users/current
